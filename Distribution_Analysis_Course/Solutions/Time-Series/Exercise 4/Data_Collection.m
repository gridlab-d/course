...
clc
format long g
read_dir=['C:\PNNL Work\Current Projects\AEP-Demo\Training Course\Time-Series 4\5 CAP\'];
disp('Time-Series 4 5CAP')
file=['reg1.csv'];
...
%% Capacitor Values (Using textscan because of the strings in the .csv)

file=['capacitor1_output.csv'];
read_file = [read_dir,file];
fid=fopen(read_file);
fgetl(fid);
fgetl(fid);
...
fgetl(fid);
textscan(fid,'%s \n');
CAP1=textscan(fid,'%s %s %s %s','delimiter',',');
for i=1:10080
    
    if strcmp(CAP1{1,2}(i,1),'CLOSED')==1
        Capacitor1(i,1)=1;
    else
        Capacitor1(i,1)=0;
    end
    
       if strcmp(CAP1{1,3}(i,1),'CLOSED')==1
        Capacitor1(i,2)=1;
    else
        Capacitor1(i,2)=0;
       end
    
           if strcmp(CAP1{1,4}(i,1),'CLOSED')==1
        Capacitor1(i,3)=1;
    else
        Capacitor1(i,3)=0;
    end
...
fgetl(fid);
textscan(fid,'%s \n');
CAP2=textscan(fid,'%s %s %s %s','delimiter',',');
for i=1:10080
    
    if strcmp(CAP2{1,2}(i,1),'CLOSED')==1
        Capacitor2(i,1)=1;
    else
        Capacitor2(i,1)=0;
    end
    
       if strcmp(CAP2{1,3}(i,1),'CLOSED')==1
        Capacitor2(i,2)=1;
    else
        Capacitor2(i,2)=0;
       end
    
           if strcmp(CAP2{1,4}(i,1),'CLOSED')==1
        Capacitor2(i,3)=1;
    else
        Capacitor2(i,3)=0;
    end
...
fgetl(fid);
textscan(fid,'%s \n');
CAP3=textscan(fid,'%s %s %s %s','delimiter',',');
for i=1:10080
    ...
    if strcmp(CAP2{1,2}(i,1),'CLOSED')==1
       ...
       if strcmp(CAP3{1,3}(i,1),'CLOSED')==1
        Capacitor3(i,2)=1;
    else
        Capacitor3(i,2)=0;
       end
    
           if strcmp(CAP3{1,4}(i,1),'CLOSED')==1
        Capacitor3(i,3)=1;
    else
        Capacitor3(i,3)=0;
    end
...
fgetl(fid);
textscan(fid,'%s \n');
CAP4=textscan(fid,'%s %s %s %s','delimiter',',');
for i=1:10080
    
    if strcmp(CAP4{1,2}(i,1),'CLOSED')==1
        Capacitor4(i,1)=1;
    else
        Capacitor4(i,1)=0;
    end
    
       if strcmp(CAP4{1,3}(i,1),'CLOSED')==1
        Capacitor4(i,2)=1;
    else
        Capacitor4(i,2)=0;
       end
    
           if strcmp(CAP4{1,4}(i,1),'CLOSED')==1
        Capacitor4(i,3)=1;
    else
        Capacitor4(i,3)=0;
    end
...
fgetl(fid);
textscan(fid,'%s \n');
CAP5=textscan(fid,'%s %s %s %s','delimiter',',');
for i=1:10080
    
    if strcmp(CAP5{1,2}(i,1),'CLOSED')==1
        Capacitor5(i,1)=1;
    else
        Capacitor5(i,1)=0;
    end
    
       if strcmp(CAP5{1,3}(i,1),'CLOSED')==1
        Capacitor5(i,2)=1;
    else
        Capacitor5(i,2)=0;
       end
    
           if strcmp(CAP5{1,4}(i,1),'CLOSED')==1
        Capacitor5(i,3)=1;
    else
        Capacitor5(i,3)=0;
    end
...
read_file = [read_dir,file];
V630=csvread(read_file,9,1,[9,1,10088,6]);
...
read_file = [read_dir,file];
V652=csvread(read_file,9,1,[9,1,10088,6]);
...
read_file = [read_dir,file];
V671=csvread(read_file,9,1,[9,1,10088,6]);
...
read_file = [read_dir,file];
V675=csvread(read_file,9,1,[9,1,10088,6]);
...
read_file = [read_dir,file];
V680=csvread(read_file,9,1,[9,1,10088,6]);
...
read_file = [read_dir,file];
OH_Losses=csvread(read_file,9,1,[9,1,10088,6]);
...
read_file = [read_dir,file];
UG_Losses=csvread(read_file,9,1,[9,1,10088,6]);
...
read_file = [read_dir,file];
XFMR_Losses=csvread(read_file,9,1,[9,1,10088,6]);%(Pa, Qa, Pb, Qb, Pc, Qc)
...
read_file = [read_dir,file];
Triplex_Losses=csvread(read_file,9,1,[9,1,10088,6]);%(P_L1, Q_L1, NA, P_L2, Q_L2, NA)
