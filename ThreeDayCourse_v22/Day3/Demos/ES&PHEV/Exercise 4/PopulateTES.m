%% This will write a bunch of thermal energy storage items for different houses
...
for loopVals=1:length(Names)
    if (PopIncVector(loopVals)<PopulationRatio)
        ...
        fprintf(fOutHandle,'object thermal_storage {\n');
        fprintf(fOutHandle,'\tname tes_%s;\n',char(Names{loopVals}));
        fprintf(fOutHandle,'\tparent %s;\n',char(Names{loopVals}));
		fprintf(fOutHandle,'\tSOC 100.00;\n');
		fprintf(fOutHandle,'\tk 0.00;\n');
		fprintf(fOutHandle,'\tdischarge_schedule_type EXTERNAL;\n');
		fprintf(fOutHandle,'\trecharge_schedule_type EXTERNAL;\n');
		fprintf(fOutHandle,'\tschedule_skew %.0f; \n', 900*(9*rand(1)-4));
		fprintf(fOutHandle,'\trecharge_time ts_recharge_schedule*1.0;\n');
		fprintf(fOutHandle,'\tdischarge_time ts_discharge_schedule*1.0;\n');
        fprintf(fOutHandle,'}\n\n');
    end
end
